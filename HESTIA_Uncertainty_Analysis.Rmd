---
title: "Toxicological database construction and uncertainty analysis of toxicological effect data used in life cycle assessment"
author: "Oskar Nyberg"
date: "`r Sys.Date()`"
output:
...
# # Running the operation with the HESTIA toxicity data set, generating a list object
  system.time(nls_output <- nls_across_all(dataset = HESTIA_BASE_EnviroTox_FILL, Plot_output = "NO", Plot_destination = "figures/SSD_plots"))
# 
...
\begin{equation}
U_r = \frac{0.5(Q97.5-Q2.5)}{logHC20_{EC10eq}}
(\#eq:eq11)
\end{equation}
...
uncertainty_ratios <- nls_output_df_OK %>% 
  ...
  mutate(quantile_range = (Q97.5-Q2.5)/2, # <- average quantile range from the central value
...
One key difference between the methodology recommended by Owsianiak et al., -@Owsianiak2019, where $\hat\mu$ and $\hat\sigma$ are the parameters used to generate $log(HC20_{EC10^{eq}})$,
and how the uncertainty analysis in the current article is performed is that we apply weighted means to species data when calculating chemical-specific $log(HC20_{EC10^{eq}})$. Data availability and spread affects M_{i} and S_{i}, which is subsequently fed into an nls model based on the cumulative distribution function \@ref(eq:eq10). This approach enables us to capture the variability in species-specific means.
 ...
 - We can compare the point data, the 
