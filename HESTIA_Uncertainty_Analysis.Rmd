---
title: "Uncertainty analysis of toxicological effect data used in life cycle assessment"
author: "Oskar Nyberg"
date: "`r Sys.Date()`"
output:
...
where $z_{0.2}$ is approximately -0.84.  
...
To model estimates for $\hat\mu$ and $\hat\sigma$, we fit a nonlinear least-squares (nls) regression model to the toxicological dose-response data using R-programming language. The script defines the cumulative normal distribution function (Eq. 10) and a self-starting function for the cumulative normal distribution to solve for the least squares residual of $\mu$ and $\sigma$, programming in R allow the use of the `nls()`function for solving cumulative normal distribution functions. Starting points the model are defined as $\mu_{start} = \frac{1}{m}\sum_{i=1}^{m}M_{i}$ and $\sigma_{start} = \frac{1}{m}\sum_{i=1}^{m}S_{i}$ respectively. In cases where only one toxicological record per species is available, $\sigma_{start}$ is not obtainable thus an arbitrary value of $\sigma_{start} = 1$ is used. Then, the function selects the unique CAS numbers and checks that there are at least 5 distinct species and 3 distinct taxonomic groups for each CAS number. If there are insufficient data, the function skips to the next CAS number. If there are sufficient data, the function calculates the mean and standard deviation of the log-transformed EC10eq values for each species within each taxonomic group. The mean values are then ranked and the self-starting function is used to fit a cumulative normal distribution to the ranked means. For each CAS number, the function outputs a list that contains the CAS number, the $log(HC20_{EC10^{eq}})$ value, $CRF_{HC20}$, the estimated parameters of the cumulative normal distribution $\mu$ and $\sigma$, the 2.5 and 97.5 percentiles of the parameter estimates, the number of species and taxonomic groups used in the analysis, the number of records (i.e., EC10 values) for the CAS number, the status of the analysis (i.e., "not enough data" or "converged"), and the raw output from the nls regression (nls_results). If specified, the function also creates a plot of the dose-response curve for each CAS number.  
...
if(knitr::is_html_output()){ 
  ...
  # Since the HTML object is not a figure, the caption and referencing does not work. Here we add a blank ggpplot with defined height and width as 0.1. as a quick-fix.
  ggplot()
...
For the final database, the HESTIA toxicological data set with EnviroTox unique records attached, we calculate `r nrow(HESTIA_HC20 %>% filter(!is.na(HC20)))` $CRF_{HC20}$ and `r nrow(HESTIA_HC50_calc %>% filter(!is.na(avlog10_HC50_EC50)))` $av_{log}HC50_{EC50}$-values out of `r nrow(HESTIA_HC20)` chemicals.  

```{r SummaryNls}
# [explain what the qnorm() function in R code is calculating]
# "The qnorm() function in R is used to compute the inverse cumulative distribution function (CDF) of the standard normal distribution. In other words, given a probability p, qnorm(p) returns the value x such that P(X <= x) = p, where X is a random variable following a standard normal distribution. This is useful when working with normally distributed data, as it allows you to find the value of x that corresponds to a particular probability or percentile in the distribution. For example, if you want to find the value x such that 95% of the observations in a normal distribution are below x, you can use qnorm(0.95) to calculate the corresponding x value."

#data set to analyze is object `HESTIA_BASE_EnviroTox_FILL`
...
# write.csv(Summary_nls, "results/nls_output_summary.csv", row.names = F)
```


To allow for comparative assessment of uncertainties at the $logHC20_{EC10eq}$ response level across the entire data set, we define uncertainty ratio ($U_r$) (or perhaps $\sigma_r$) given by 0.5 * 2.5 to 97.5 quantiles range divided by the central value ($logHC20_{EC10eq}$) as  

\begin{equation}
U_r = \frac{0.5(Q2.5-Q97.5)}{logHC20_{EC10eq}}
(\#eq:eq11)
\end{equation}

The 2.5% and 97.5% quantiles are unequally distanced from the central $logHC20_{EC10eq}$ value, which is why we generate an "artificial" distance between the central value and each quantile. The calculated $U_r$ tells us for the majority of data.  

```{r}
## select only substances with enough data to fit a model to
nls_output_df_OK <- nls_output_df %>% 
  filter(status == "OK")

uncertainty_ratios <- nls_output_df_OK %>% 
  select(CAS.Number, log_HC20EC10eq, Q2.5, Q97.5, n_recs, n_sp, n_tax.grp) %>% 
...
summary_uncertainty_ratios <- autofit(flextable(uncertainty_ratios %>% 
  filter(!is.na(uncertainty_ratio)) %>% 
  summarise_at(vars(uncertainty_ratio),
              list(min=min, 
                    Q1=~quantile(., probs = 0.25, na.rm = T, digits = 2),
                    median=median, 
                    Q3=~quantile(., probs = 0.75, na.rm = T, digits = 2),
                    max=max))))

summary_uncertainty_ratios %>% 
  set_caption("Uncertainty ratios distribution across the data set with minimum, 25%-quantile (Q1), median, 75%-quantile(Q3) and maximum") %>% 
  fontsize(., size = 9, part = "all")
  

### joining the uncertainty ratios with physchem data
Uncert_analysis_df <- uncertainty_ratios %>% 
  left_join(
    x = ., 
    y = FULL_PHYSCHEM %>% 
      select(CAS.Number, MW.g.mol, pKaChemClass, Group, Substance_type), 
    by = "CAS.Number"
  )

# Histogram detailing the distribution of relative uncertainty of substances at the HC20 response level
histogram_uncertainty_ratio <- Uncert_analysis_df %>% 
  ggplot(aes(uncertainty_ratio))+
  geom_histogram(aes(), bins = 100)+
  scale_x_log10() +
  xlab("Uncertainty ratio (Ur) presented at log10 scale")

# trimmed_uncert <- Uncert_analysis_df %>% 
#   arrange(desc(uncertainty_ratio)) %>% 
#   slice(20:nrow(Uncert_analysis_df))
# trimmed_uncert %>% 
#   ggplot(aes(uncertainty_ratio))+
#   geom_histogram(aes(), bins = 30)+
#   #scale_x_log10() +
#   xlab("Ur (NOT log10 scale)")

# Histogram detailing the number of records for chemicals with a modeled HC20 response level
histogram_n_recs <- nls_output_df_OK %>% 
  ggplot(aes(n_recs))+
  geom_histogram(aes()) +
  scale_x_log10()
 

# Looking into uncertainty ratio values that are crazy big. 
# Uncert_analysis_df %>%
#   arrange(desc(uncertainty_ratio))
# 
# HESTIA_BASE_EnviroTox_FILL %>% 
#   filter(CAS.Number == "7786-81-4")
# 
# EnviroTox_DB %>% 
#   filter(CAS.Number == "59-87-0")
#   filter(Source == "Ecotoxicol. Environ. Saf.8(2): 101-105")

# This CAS number turns out very problematic with the current approach. When logHC20EC10eq value is very close to 0 the low quantile range is still much bigger than the absolute value of logHC20EQ10eq and the ratio turns out huge. 
# CAS       logHC20EC10eq Q2.5              Q97.5             n_recs  n_sp    n_tax.grp   quantile range  Uncert-Ratio  
# 7786-81-4	0.0002409813	0.0148134988	    -1.644669e-02	    279	    59	    9	          0.015630093	    64.86020233	
# 541-73-1	0.0020381073	0.0096607551	    -6.241808e-02	    90	    11	    4	          0.036039417	    17.68278684

```


**The approach to produce a ratio is flawed. When logHC20EC10eq value is close to 0 the low quantile range is still much bigger than the absolute value of logHC20EQ10eq and the ratio turns out huge.** How can we change this approach?  
```{r}
# Boxplot of relative uncertainty in the HC20 response level for substances groups
Boxplot_uncertainty_per_group <- Uncert_analysis_df %>% 
  ggplot(aes(x = Group, y = uncertainty_ratio)) +
...
When exploring the relationship between the number of records or the molecular weight of substances to the uncertainty ratio of chemicals we find, unsurprisingly, that the more data points each nonlinear least squares model are modeled after, the lower the uncertainty is for corresponding $log(HC20_{EC10^{eq}})$ point (Figure \@ref(fig:uncert)).  

```{r uncert, fig.dim = c(12, 6), fig.cap = "Uncertainty analysis figures (TEST CAPTION)"}
n_rect_to_uncertainty <- Uncert_analysis_df %>% 
  ggplot(aes(x = n_recs, y = uncertainty_ratio)) +
  geom_point(aes(color = Group), alpha = 0.5) +
  geom_smooth()+
  annotation_logticks() +
  scale_x_log10() +
  scale_y_log10(breaks = 10^(-2:3), labels = scales::number_format(accuracy = 0.01)) +
  xlab("Number of records per chemical (log-scale)") +
  ylab("Uncertainty ratio (log-scale)") +
  #labs(tag = "a)") +
  theme_bw() +
  theme(legend.position = "none") #+ 
  #ggtitle("Relationship between the number of records and \nuncertainty ratio for different groups of chemicals")

n_rect_to_uncertainty

# # Uncertainty ratio variability as a result of molecular weight  
# mol_w_to_uncertainty <- Uncert_analysis_df %>% 
#   ggplot(aes(x = MW.g.mol, y = uncertainty_ratio)) +
#   geom_point(aes(color = Group), alpha = 0.5) +
#   #geom_smooth()+
#   scale_x_log10() +
...
#   theme_bw() +
#   theme(legend.position = "none") #+
#   #ggtitle("Exploring the relationship between the molecular weight and \nuncertainty ratio for different groups of chemicals")
# 
# grid.arrange(n_rect_to_uncertainty, mol_w_to_uncertainty, ncol=2)

# Lets run a linear model to test the relationship between n_recs and uncertainty ratio:
# n_recs_x_uncertainty_lm <- lm(n_recs~uncertainty_ratio, Uncert_analysis_df, na.action = na.exclude)
# summary(n_recs_x_uncertainty_lm)
...
QSAR_plot_HQ <- QSAR_emp_abs_diff_df %>% 
    ...
    #scale_y_log10() +
    #scale_x_log10() +
    geom_circle(aes(x0 = -5, y0 = -1, r = 1.5), color = "red", fill = NA) +
...
hist_abs_diff <- QSAR_emp_abs_diff_df %>%
  ...
  # scale_x_log10()
#   
  # slice(QSAR_emp_abs_diff_df %>%
  #   arrange(HC20EC10eq), 1:6) #%>%
  #   pull(CAS.Number)
...
### Discussion  
By gathering physicochemical and ecotoxicological effect data for a set of `r nrow(HESTIA_chem_list_slim)` chemicals through querying OECD QSAR Toolbox software, we have been able to calculate concentration-response slope factors at the 20% response level ($CRF_{HC20}$) for `r nrow(HESTIA_HC20 %>% filter(!is.na(HC20)))` chemicals, assess the uncertainty at the $log(HC20_{EC10^{eq}})$ point of the SSD curve for `r nrow(nls_output_df_OK)` chemicals and gather `r nrow(FULL_PHYSCHEM %>% filter(CanonicalSMILES != ""))` physicochemical records required for characterization of chemicals with USEtox 2.1 methodology. The present data set is not a fully comprehensive database for all potentially harmful substances from open sources, 16797 substances is only a small part of the full spectrum of man-made substances released into the environment and within this subset of data toxicological effect data are available for `r round(100*(nrow(read.csv("data/RAW_DB/HESTIA_HC20_DB_raw_toxdata.csv") %>% distinct(CAS.Number))/nrow(HESTIA_chem_list_slim)), 0)`% of substances queried.  More substances need to be tested, across a more diverse set of species than the standard *Danio rerio*, *Daphnia magna*, and *Pseudokirchinella subcapitata* for the current characterization methodology to be able to include a larger set of potentially harmful substances.    
