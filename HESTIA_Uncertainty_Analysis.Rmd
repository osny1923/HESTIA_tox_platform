...
```{r GeoStDev, fig.dim = c(13, 15), fig.cap = "Geometric standard deviation of probability distribution ranges based on Monte Carlo simulations per number of effect data records per chemical.  Curve fitted using loess smoothing. Geo.StDev = Geometric standard deviation."}
...
A well known problem for probabilistic assessments of uncertainty in pesticide ecotoxicity is the variation of sensitivity between species to pesticides [@warren2010application], it is in the nature of these types of substances. So we might assume that pesticides will have large uncertainties, which is indeed the case for the insecticide "Novaluron" and herbicide "Pyroxsulam" where the geometric standard deviation is extreme (5.1 and 4.8 respectively) (figure \@ref(fig:GeoStDev)). This brings up the issue of how to properly assess pesticides:  
