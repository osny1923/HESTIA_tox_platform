...
To allow for comparative assessment of uncertainties at the $log(HC20_{EC10^{eq}})$ response level across the entire data set, we apply Monte Carlo simulations using the average and standard deviation results from the nls model 

Vänta. Om vi först kör monte carlo för att uppskatta osäkerheterna i ett dataset, och sedan kör monte carlo... kan man inte bara köra monte carlo på icke-vägd data?
...
## Select only data with n_sp >5 & n_tax.grp >3
required_data_nls <- nls_output_df_OK %>% 
  filter(n_sp >= 5 & n_tax.grp >=3)

n_rec_to_uncertainty <- nls_output_df_OK %>% 
  ggplot(aes(x = n_recs, y = Geo_St.Dev)) +
  geom_point(aes(), alpha = 0.5) +
  geom_smooth(se = TRUE) + # metod:mgcv::gam() (Generalized additive models with integrated smoothness estimation) is used with formula = y ~ s(x, bs = "cs") with method = "REML".
  #annotation_logticks() +
  scale_x_log10() +
...
While the methodology recommended by Owsianiak et al., -@Owsianiak2019, where the arithmetic mean of vectors $M_{i}$ and $S_{i}$ respectively are used to calculate a $log(HC20_{EC10^{eq}})$-value, the present approach fits the cumulative normal distribution to the vector of $M_{i}$, allowing to account for the variance across all species data. This is crucial for a fair estimate of uncertainty of the $log(HC20_{EC10^{eq}})$-value, as data availability varies greatly across species, with few species being over represented in the dataset (e.g `r knitr::combine_words(sp_count_tbl[1:3,1])` representing `r round((sum(sp_count_tbl[1:3,2])/nrow(HESTIA_BASE_EnviroTox_FILL))*100, 2)` % of the dataset).  
...
**Conclusions**  
 - We have been able to calculate the uncertainties at the $log(HC20_{EC10^{eq}})$ point using Monte Carlo simulations for `r nrow(nls_output_df %>% filter(status== "Convergence"))` chemicals, yet only fewer than 10% of the original data set with 16797 chemicals.    
