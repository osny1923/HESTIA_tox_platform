...
The following document provides detailed descriptions to the construction of the HESTIA Ecotoxicological database constructions. Data is attached as a Microsoft Excel file (Supplementary Data) and R code is available at https://github.com/osny1923/HESTIA_tox_platform.  
...
### 3.1.1 Data gathering and processing
...
### 3.1.2 Substance type annotations
...
### 3.1.3 Categorizing use classification and chemical types
...
sptab1 <- removedSpecies %>% slice(1:60)
sptab2 <- removedSpecies %>% slice(61:120)
sptab3 <- removedSpecies %>% slice(121:180)
sptab4 <- removedSpecies %>% slice(181:233) %>% 
  rbind(., 
        data.frame(Test.organisms..species. = rep("", 7), 
                   n = rep("", 7)))
...
## 3.3 Curation of ecotoxicological data
...
### 3.3.1 Adjusting test concentrations reported as a range, 
...
manCur %>% 
  set_caption("Manually removed studies from the dataset") %>% 
  set_table_properties(layout = "autofit") %>% 
  flextable::fontsize(., size = 9, part = "all")
```

  
### 3.3.3 Test medium selection
```{r}
...
### 3.3.4 Selecting ecologically relevant effect criterions   
...
flextable::flextable(RawToxData %>% 
  ...
  mutate("Ecological relevance" = case_when(Effect %in% Included_criterions ~ "YES", TRUE ~ "NO")) %>% 
    rename("Number of records" = n)) %>% 
  set_caption("Number of records included and removed depending on test effect criterion") %>% 
  set_table_properties(layout = "autofit") %>% 
  flextable::fontsize(., size = 9, part = "all")
```
 

### 3.3.5 Selecting relevant taxonomic annotations  
...
### 3.3.6 Effect concentration and unit harmonization
...
### 3.3.8 Assign acute or chronic test definitions.
