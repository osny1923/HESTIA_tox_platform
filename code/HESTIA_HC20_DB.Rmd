...
nodes = data.frame("name" = c(
   ...
   paste("1) n = ", nrow(HESTIA_HC20_DB) - nrow(HESTIA_HC20_DB_qualifier_filter), sep = ""), 
   paste("2) n = ", nrow(HESTIA_HC20_DB_qualifier_filter) - nrow(HESTIA_HC20_DB_Control.Type_filter), sep = ""), 
   paste("3) n = ", nrow(HESTIA_HC20_DB_Control.Type_filter) - nrow(HESTIA_HC20_DB_water_filter), sep = ""), 
   paste("4) n = ", nrow(HESTIA_HC20_DB_water_filter) - nrow(HESTIA_HC20_DB_effect_crit_filter), sep = ""), 
   paste("5) n = ", nrow(HESTIA_HC20_DB_effect_crit_filter) - nrow(HESTIA_HC20_DB_taxonomy_filter), sep = ""), 
   paste("6) n = ", nrow(HESTIA_HC20_DB_unit_and_value_conv) - nrow(HESTIA_HC20_DB_effect_conc_filter), sep = ""), 
   paste("7) n = ", nrow(HESTIA_HC20_DB_val.unit_filter) - nrow(HESTIA_HC20_DB_time.unit_filter), sep = ""),
...
HESTIA_BASE_dat <- read.csv("../results/HESTIA_EC10eq_DB.csv") %>% 
  ...
  #mutate(CAS.Number = as.factor(CAS.Number))
...
# load the nls function
 source("../code/least_squares_fit_model_code.R")
system.time(nls_output_subset <- nls_across_all(dataset = HESTIA_BASE_dat %>% filter(CAS.Number %in% dataRichNoConverge), Plot_output = "NO", MC_n = 1e+05, Plot_destination = "../figures/SSD_plots"))

#Transform into a dataframe - removing the list of lists containing full nls-output
nls_output_df_subset <- as.data.frame(do.call(cbind, nls_output_subset[1:20])) %>%
 mutate(across(c(2:19), ~ as.numeric(.x)),
        CAS.Number = as.factor(CAS.Number),
        status = as.character(status))

failConverge <- nls_output_df_subset %>% 
  filter(GStDev == 0) %>% pull(CAS.Number)

data_from_bad_nls_starts %>% 
  filter(CAS.Number %in% failConverge) %>% 
  mutate(sp_mean = log10((10^sp_mean)*1e6)
         )

#HESTIA_BASE_dat %>% filter(CAS.Number == "2782-70-9" & Species == "Lepomis macrochirus")
```
...
# Selection for the SHINY-app
  select(c(
    ...
    Molecular.formula, Predefined.substance.type, Substance_type, 
    Group, Subgroup, Heavy.Metals, Halogenated, definition, 
...
openxlsx::writeData(Supp, sheet = "HESTIA_INVENTORY", x = read.csv("../data/excel_references/pesticideAI.csv"), startCol = 1, startRow = 1, colNames = T, rowNames = F)
# CAS SMILES matching
...
# Final Database
openxlsx::writeData(Supp, sheet = "FULL_DATABASE", x = HESTIA_BASE_dat, startCol = 1, startRow = 1, colNames = T, rowNames = F)
