...
```{r}
### OBSOLETE and SLOW for-loop code:
# Uncertain_norm_dataset_func <- function(x) {
#   # Make a CAS.Number vector to loop over
#   H_CAS <- x %>% 
#     distinct(CAS.Number) %>% pull()
#   UC_df <- data.frame(matrix(NA, nrow = length(H_CAS), ncol = 8))
#   names(UC_df)[1:8] <- c("CAS.Number", "n", "Q0.05", "Q0.95", "mean_EC10eq", "median_EC10eq", "sd_EC10eq", "CV_EC10eq")
#   
...
## Not functional yet. need to adapt to reworked datasets!
```{r FinalDataset}
...
# Selection for the SHINY-app
  select(c(
    CAS.Number, CanonicalSMILES, PesticideAI_name, Molecular.formula, Predefined.substance.type,
    Substance_type, Group, Subgroup, Heavy.Metals, Halogenated, definition, n_data_recs, n_species, sum_extrapolations, n_Taxonomy.Group, 
    mean_EC10eq, Sd_EC10eq, HC20EC10eq, HC20,
    MW.g.mol, pKaChemClass,
...
# Selection for the USEtox input
  # select(c(
  #   CAS.Number, PesticideAI_name, Group, Subgroup, 
  #   MW.g.mol, pKaChemClass, pKa.gain, pKa.loss, 
  #   Kow_L.L, Koc_L.kg_MCI, kH25C_Pa.m3.mol, Vapor.Pressure_Pa, Sol_mg.L,
  #   KdegA, KdegW, KdegSd, KdegSl, HC20EC10eq, BAF_L.Kg
  #   # Molecular.formula, Predefined.substance.type, CanonicalSMILES, Substance_type, Heavy.Metals, Halogenated, definition, n_data_recs, n_species, sum_extrapolations, n_Taxonomy.Group, 
  #   # mean_EC10eq, Sd_EC10eq,  HC20,
  #   )
  # )

write.csv(HESTIA_envirotox_cfs, "../results/HESTIA_HC20_dataset_USEtox_format.csv", row.names = F)
```
